...
const doubleBind = (actions, api, plugin, actionOptions) => {
  const {
    traceId
  } = actionOptions;
  ...
  if (boundPluginActionCreators[plugin.name + api + traceId]) {
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      const boundActionCreator = actions[key];

      if (typeof boundActionCreator === `function`) {
        doubleactions[key] = (...args) => {
    ...
    boundPluginActionCreators[plugin.name + api + traceId] = doubleactions;
...
const runAPI = (plugin, api, args, activity) => {
  ...
  if (gatsbyNode[api]) {
    ...
    const actions = bindActionCreators(availableActions, store.dispatch);
    const doubleactions = doubleBind(actions, api, plugin, Object.assign({}, args, {
